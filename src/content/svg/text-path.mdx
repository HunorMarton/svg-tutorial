...
suggestions: [clip-path, gradient, clock, interaction]
---

import Final from "../../components/markdown/layouts/Final.astro";

Drawing shapes is not the only use case for paths. We can also use them to render text along an invisible path. We can define a path in the definitions section and use it in a `element:textPath` element to make the text go around the circle. Here we use arc again, but you can use any other path and the text will follow the stroke.
