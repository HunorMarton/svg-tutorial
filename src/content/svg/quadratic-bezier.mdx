...
description: Learn how to set the control point when drawing a quadratic Bézier curve with SVG. In this tutorial, we learn a basic curve type and draw a smiley face and a wavy tree.
suggestions: [cubic-bezier, bear, arc, animation-on-hover]
---
...
The [path](/svg/basic-path) element becomes really powerful when we start using curves, such as the quadratic Bezier curve. For this curve, we also have to set a control point apart from the endpoint.
...
You can create a [smiley](/smile) icon with a quadratic Bezier curve. Look at this happy face! Hover over the coordinates in the code or on the image to see how they are positioned.

In this image, we also introduce a new element, the `element:ellipse`. An ellipse behaves like a circle, except it has not one but two radiuses: a horizontal one and a vertical one.
