...
Generating [images with central symmetry](/svg/transform) is one of the many use cases of rotating image elements. In this example, we create a clock that shows the actual time. We draw the clock with SVG and use JavaScript to turn the hour and minute handles in the right direction.
