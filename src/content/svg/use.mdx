...
Then, we can move the lines to their correct position with the `transform` command in the same way we did with [the star example](/svg/rotate-and-translate), except that this time, we increment the rotation by 45 degrees for each ray.
