...
description: In this example, learn how to animate an SVG bell on mouse hover using CSS. First, we draw the bell with SVG paths and then add a keyframe animation to rotate the bell and the bell clapper back and forth.
...
We can define keyframe animations for SVG elements with CSS. In this example, we animate the `property:transform` property with keyframes. Here, we trigger this effect on mouse hover.
...
Now, let's add an `property:id` for the elements we move. We assign an id to the group containing the whole bell and one for the bell clapper.
