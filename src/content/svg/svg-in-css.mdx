...
suggestions: [css-animation, animation-on-hover, text-path, clip-path]
---
...
The inlined SVG above is simply an X as follows (except that the `property:stroke` property is set to black in this case for visibility). The gray circle around it is made with CSS (by setting the `background-color` and the `border-radius` properties).
